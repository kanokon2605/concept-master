...
<body>
  ...
  <div class="dashboard-wrapper">
      ...
      <div class="container-fluid dashboard-content ">
       ...
       <?php 
        include"config.inc.php";
         if(isset($_GET['edit'])){
           $sql_edit = "SELECT * FROM c_brand where id_brand=$_GET[edit]";
                  $query_edit = $conn->query($sql_edit); 
                 $result_edit = $query_edit->fetch_assoc();
       }
       
       if(isset($_GET['del'])){
           $sql_del = "DELETE FROM c_brand WHERE c_brand.id_brand = $_GET[del]";
                  $query_del = $conn->query($sql_del); 
      echo "<script type='text/javascript'>";
     //echo  "alert('สำเร็จ');";
      echo "window.location='brand.php';";
      echo "</script>";
                
       }
       if(isset($_POST['add'])){
     include"config.inc.php";
     $sql = "INSERT INTO c_brand (id_brand, name_brand) VALUES (',$_POST[id_brand]', '$_POST[name_brand]')";
    ...
    if(isset($_POST['edit'])){
    include"config.inc.php";
    $sql = "UPDATE c_brand SET name_brand = '$_POST[name_brand]' WHERE c_brand.id_brand = $_POST[id_brand]";
            $query = mysqli_query($conn,$sql);
    ...
    ?>
                       <div class="box box-primary">
                            ...
                            <input name="id_brand" type="hidden" value="<?=@$result_edit['id_brand'];?>">
                                <div class="box-body">
                                   <div class="row">
                                   <div class="col-xs-6">
         <input type="text" name="name_type" class="form-control"  value="<?=@$result_edit['name_brand'];?>" placeholder="name" required>
                                    </div>
                                    <div class="col-xs-3">
        ...
        include"config.inc.php";
        $i=0;
           $sql = "SELECT * FROM c_brand ORDER BY id_brand DESC";
...
<td>
<a href= "?del=<?=$result['id_brand'];?>" class="btn btn-danger btn-sm"  onClick="return confirm('กรุณายืนยันการลบอีกครั้ง !!!')"> ลบ </a>
<a href="?edit=<?=$result['id_brand'];?>" class="btn btn-info btn-sm" > แก้ไข </a>
